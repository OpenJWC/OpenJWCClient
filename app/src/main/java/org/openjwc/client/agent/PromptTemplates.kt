...
object PromptTemplates {
     ...
     * 系统提示词。
     *
     * 结构：角色人设 → 最高优先守则（不可被用户要求覆盖）→ 回答要求。
     * 当前日期等运行信息由 [metadata] 追加在提示词末尾。
     */
    val SYSTEM_PROMPT = """
    ...
    fun metadata(
        ...
        lastCrawlMillis: Long?,
        now: ZonedDateTime,
        sources: List<Pair<String, String>> = emptyList(),
    ): String {
        val last = lastCrawlMillis?.let {
            java.time.Instant.ofEpochMilli(it).atZone(now.zone)
                .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))
        } ?: "尚无完整成功抓取记录"
        val range = if (catalog.total > 0) {
            "${catalog.firstDay.orEmpty()} 至 ${catalog.lastDay.orEmpty()}"
        } else {
            "空库"
        }
        val sourceLine = if (sources.isEmpty()) {
            "尚未订阅任何数据源。"
        } else {
            "已订阅数据源（source_id=名称）：" +
                sources.joinToString("、") { "${it.first}=${it.second}" } + "。"
        }
        return "当前时间：${now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME)}；时区：${now.zone}。" +
    ...
    /**
     * 当前用户问题。引用的资讯不再拼在这里（改由 system 的「引用的资讯」块统一给出），
     * 保持用户原话干净，也避免把注入内容伪装成用户指令。
     */
    fun userQuery(request: AgentRequest): String = request.query

    /** 日报默认提示词（对齐后端 `daily_prompt` 默认值）。 */
    const val DAILY_PROMPT =
        "总结今天发布的教务资讯，区分事实与建议，指出重要截止日期以及对学生有用的资讯，并给出资讯标题与链接。"

    /** 日报分批任务：让模型逐条读所选资讯。 */
    fun dailyBatchQuery(day: String, prompt: String = DAILY_PROMPT): String =
        "这是 $day 日报的一批资讯。$prompt\n请逐条阅读所选资讯，给出有证据的简洁摘要。"

    /** 日报合并任务。 */
    fun dailyMergeQuery(day: String, parts: String): String =
        "请根据以下 $day 分批资讯摘要合并成一篇日报，保留来源引用，去重且不要添加没有证据的事实。\n$parts"

    /**
     * 正常完成时的收束指令：由一次禁用工具的流式请求产出最终答案。
     * 注意要同时覆盖「检索类问题」与「寒暄/身份/功能类问题」，否则后者会被答成「没有查到」。
     */
    fun finalInstruction(): String =
        "请给出最终回答。若本轮读到了具体资讯，请给出标题、栏目、日期与链接（Markdown 形式）并保持客观；" +
            "若这个问题不需要资讯（寒暄、身份、功能、课程安排等），直接自然地回答就好，不必提「没有查到」。" +
            "不确定就说不确定，不要编造。"

    fun limitInstruction(reason: String): String =
